...
Enter customer first name: <input type="text" name="term" /><br />  
...
$query = "SELECT * FROM customer WHERE fname LIKE '%".$term."%'"; 
$result = mysql_query($query); 

echo "<table>";
while($row = mysql_fetch_array($result)){   //Creates a loop to loop through results
echo "<tr><td>" . $row['cust_id'] . "</td><td>" . $row['fname']  . "</td><td>" .  $row['lname'] . "</td><td>" . $row['age'] . "</td><td>" . $row['contact_no'] ."</td><td>" . $row['email'] . "</td></tr>";  //$row['index'] the index here is a field name
}
} 
