...
Enter room ID: <input type="text" name="term" /><br />  
...
$query = "SELECT * FROM rooms WHERE room_no LIKE '%".$term."%'"; 
$result = mysql_query($query); 

echo "<table >";
while($row = mysql_fetch_array($result)){   //Creates a loop to loop through results
echo "<tr><td>" . $row['room_no'] . "</td><td>" . $row['room_type']  . "</td><td>" .  $row['price'] . "</td><td>" . $row['hotel_id'] ."</td></tr>";  //$row['index'] the index here is a field name
}
}
